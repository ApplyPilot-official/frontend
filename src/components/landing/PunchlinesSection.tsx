...
/* ──────────────────────────────────────────
   Hero Stat — the big "$1/day → $200K" banner
   ────────────────────────────────────────── */
function HeroStatBanner() {
    ...
    return (
        ...
        >
            {/* Glow background */}
            <div className="absolute inset-0 bg-gradient-to-r from-neon-blue/10 via-neon-violet/15 to-neon-emerald/10 rounded-3xl blur-[80px]" />

            <div className="relative glass-card rounded-3xl p-8 sm:p-12 overflow-hidden border border-white/[0.08]">
                {/* Corner sparkles */}
                <div className="absolute top-4 left-4 text-xl opacity-60">✨</div>
                <div className="absolute bottom-4 right-4 text-xl opacity-60">✨</div>
                <div className="absolute top-0 right-0 w-64 h-64 bg-neon-violet/10 rounded-full blur-[100px]" />
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-neon-blue/10 rounded-full blur-[100px]" />
                ...
                <div className="relative text-center">
                    <motion.p
                        ...
                        className="text-sm sm:text-base text-slate-400 mb-4 tracking-wide"
                    >
                        Think about it...
                    </motion.p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-5 mb-6">
                        ...
                        <motion.span
                            ...
                            style={{ backgroundImage: "linear-gradient(135deg, #10b981, #34d399, #6ee7b7)" }}
                        >
                            $200K+
                        </motion.span>
                        <motion.span
            ...
            <div className="relative h-[180px] sm:h-[160px] flex items-center justify-center overflow-hidden">
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white leading-tight mb-1">
                ...
                {punchlines.map((_, i) => (
                    <button
                        ...
                        className={`transition-all duration-300 rounded-full ${i === currentIndex
                            ? "w-8 h-2 bg-gradient-to-r from-neon-blue to-neon-violet"
...
/* ──────────────────────────────────────────
   Scrolling Stats Ticker (infinite marquee)
   ────────────────────────────────────────── */
        ...
        <div className="relative overflow-hidden py-6 mb-16">
            ...
            <div className="flex animate-scroll-left mb-3">
                {[...scrollingLines, ...scrollingLines].map((line, i) => (
                    <div
                        key={`a-${i}`}
                        className="shrink-0 mx-2 px-5 py-2 rounded-full glass border border-white/[0.06] text-sm font-medium text-slate-300 whitespace-nowrap hover:border-neon-blue/30 hover:text-white transition-all duration-300 cursor-default"
                    >
                        {line}
                    </div>
                ))}
            ...
            <div className="flex animate-scroll-right">
                {[...scrollingLines.slice().reverse(), ...scrollingLines.slice().reverse()].map((line, i) => (
                    <div
                        key={`b-${i}`}
                        className="shrink-0 mx-2 px-5 py-2 rounded-full glass border border-white/[0.06] text-sm font-medium text-slate-400 whitespace-nowrap hover:border-neon-violet/30 hover:text-white transition-all duration-300 cursor-default"
                    >
                        {line}
                    </div>
                ))}
...
/* ──────────────────────────────────────────
   Impact Grid — bold stat cards
   ────────────────────────────────────────── */
const impactCards = [
    {
        stat: "24/7",
        label: "Your AI never sleeps",
        desc: "Applications sent round the clock, even at 3 AM.",
    ...
    },
    {
        stat: "150+",
        label: "Apps per day",
        desc: "What takes you a week, we do before lunch.",
    ...
    },
    {
        ...
        desc: "ATS-optimized resumes that actually get read.",
        gradient: "from-neon-emerald to-green-400",
        icon: "📈",
    },
    {
        stat: "7 days",
        label: "To first interview",
        desc: "Most users hear back within their first week.",
        ...
        <div ref={ref} className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
            {impactCards.map((card, i) => (
                <motion.div
                    ...
                    className="relative glass-card rounded-2xl p-5 sm:p-6 overflow-hidden group cursor-default border border-white/[0.06] hover:border-white/[0.12] transition-colors"
                >
...
/* ──────────────────────────────────────────
   Why This Works — typewriter reveal section
   ────────────────────────────────────────── */
const whyLines = [
            ...
            <div className="max-w-2xl mx-auto">
                {whyLines.map((line, i) => (
                    <motion.div
                        ...
                        className={`flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 py-4 ${i === 0 ? "border-b border-white/5" : ""
                            }`}
                    >
                        <span className="text-sm text-slate-500 font-mono">{line.prefix}</span>
                        <span className={`text-xl sm:text-2xl font-bold ${line.bad ? "text-red-400 line-through decoration-red-500/60" : "gradient-text"
                            }`}>
                            {line.value}
                        </span>
...
export default function PunchlinesSection() {
    return (
        <section className="py-20 sm:py-28 relative overflow-hidden">
            {/* Background effects */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-neon-violet/[0.04] rounded-full blur-[200px]" />
            <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-neon-blue/[0.05] rounded-full blur-[150px]" />
            ...
            <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* 2. Rotating punchlines */}
                <PunchlineCarousel />

                {/* 3. Scrolling pill ticker */}
                <ScrollingStatsTicker />

                {/* 4. Impact stat cards */}
                <ImpactGrid />

                {/* 5. Why it works comparison */}
                <WhyItWorks />
