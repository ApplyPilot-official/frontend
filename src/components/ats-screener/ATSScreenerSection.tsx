...
function getScoreBg(score: number) {
    ...
    return "bg-red-50 border-red-500/20";
}
    ...
    return (
        <div className="relative" style={{ width: size, height: size }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-surface-500" style={{ fontSize: size * 0.1 }}>/ 100</span>
            </div>
        </div>
    );
}
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-3">
                <div>
                    <h3 className="text-surface-950 font-semibold text-sm">{result.system}</h3>
                    <p className="text-xs text-surface-500">{result.vendor}</p>
                </div>
                ...
                <div className="flex-1 ml-4 space-y-1">
                    {[
                        ...
                        <div key={item.label} className="flex items-center gap-2">
                            <span className="text-[10px] text-surface-500 w-14">{item.label}</span>
                            ...
                            <span className="text-[10px] text-surface-600 w-6 text-right">{item.score}</span>
                        </div>
                    ))}
                </div>
            </div>
...
function DetailView({ result, onClose }: { result: ScoreResult; onClose: () => void }) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
            className="bg-white rounded-2xl p-6 border border-surface-300">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <ScoreCircle score={result.overallScore} size={56} />
                    <div>
                        <h3 className="text-surface-950 font-bold text-lg">{result.system}</h3>
                        <p className="text-xs text-surface-500">{result.vendor}</p>
                    </div>
                </div>
                <button onClick={onClose} className="text-surface-600 hover:text-surface-800 text-xl">✕</button>
            </div>
            <div className="space-y-4">
                {dimensions.map((dim) => (
                    <div key={dim.label} className="p-4 bg-white/[0.02] border border-surface-200 rounded-xl">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-semibold text-surface-700">{dim.label}</span>
                        ...
                        <ul className="space-y-1">
                            {dim.details.map((d, i) => (
                                <li key={i} className="text-xs text-surface-600 flex items-start gap-1.5">
                                    <span className="text-surface-600 mt-0.5">•</span>{d}
                                </li>
                            ))}
                        </ul>
                    </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mb-6">
                <h2 className="text-xl font-bold text-surface-950 flex items-center gap-2">
                    🎯 ATS Screener
                    <span className="text-xs font-medium text-surface-500 bg-surface-200 px-2 py-0.5 rounded-full border border-surface-300">6 Platforms</span>
                </h2>
                ...
                {["Upload", "Parse", "Scan", "Results"].map((label, i) => (
                    <div key={label} className="flex items-center">
                        <div className={`w-8 h-8 flex items-center justify-center rounded-full text-xs font-bold transition-all duration-500 ${step > i ? "bg-emerald-500/15 border-2 border-emerald-500/40 text-emerald-400" :
                            step === i ? "bg-primary-500/15 border-2 border-primary-500/40 text-primary-500" :
                                "bg-surface-200 border-2 border-surface-300 text-surface-600"
                            }`}>
                            {step > i ? "✓" : i + 1}
                        </div>
                        {i < 3 && <div className={`w-12 h-0.5 transition-all duration-500 ${step > i ? "bg-gradient-to-r from-emerald-500/40 to-primary-500/30" : "bg-white/10"}`} />}
                    </div>
                ))}
            </div>
            ...
            {/* Upload Area */}
            <div
                className={`relative p-8 rounded-2xl border-2 border-dashed cursor-pointer text-center transition-all ${isDragging ? "border-primary-500 bg-primary-500/5 shadow-lg shadow-primary-500/10" :
                    file ? "border-emerald-500/30 bg-emerald-500/5 border-solid" :
                        "border-surface-300 bg-white/[0.02] hover:border-surface-400 hover:bg-white/[0.04]"
                    }`}
            ...
            >
                ...
                {isParsing ? (
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-surface-950 font-semibold">Parsing Your Resume...</p>
                        <p className="text-xs text-surface-500">Extracting text and metadata</p>
                    </div>
                ) : file ? (
                    <div className="flex items-center justify-center gap-4">
                        ...
                        <div className="text-left">
                            <p className="text-surface-950 font-semibold">{file.name}</p>
                            <p className="text-xs text-surface-500">{(file.size / 1024).toFixed(0)} KB{parsedText ? ` · ${parsedText.split(/\s+/).length} words extracted` : ""}</p>
                        </div>
                    ...
                    <div className="flex flex-col items-center gap-3">
                        <div className="w-16 h-16 bg-primary-500/8 border border-primary-500/15 rounded-2xl flex items-center justify-center">
                        ...
                        <p className="text-surface-950 font-semibold">Drag & Drop Your Resume</p>
                        <p className="text-xs text-surface-500">or click to browse. PDF or DOCX, max 10MB.</p>
                        <div className="flex gap-2">
                            {[".PDF", ".DOCX"].map((fmt) => (
                                <span key={fmt} className="text-[10px] font-semibold text-surface-500 bg-white/[0.03] border border-surface-300 px-2 py-0.5 rounded">{fmt}</span>
                            ))}
                        </div>
                    </div>
                )}
            ...
            {file && (
                <div className="mt-4">
                    <button onClick={() => setShowJD(!showJD)}
                        className="text-xs text-primary-500 hover:text-surface-800 flex items-center gap-1.5 transition-colors">
                        ...
                        {showJD && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                                <textarea
                                    ...
                                    className="w-full mt-3 p-4 bg-white border border-surface-300 rounded-xl text-sm text-surface-700 placeholder-slate-600 resize-none focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500/30"
                                    rows={5}
                                />
                            </motion.div>
                        )}
            ...
            {/* Error */}
            {error && (
                <div className="mt-4 p-3 bg-red-50 border border-red-500/20 rounded-xl flex items-center gap-2 text-sm text-red-400">
                ...
                <div className="flex gap-3 mt-6">
                    {hasResults && (
                        <button onClick={handleReset}
                            className="px-5 py-2.5 text-sm font-medium text-surface-600 bg-surface-200 border border-surface-300 rounded-xl hover:text-surface-800 hover:border-surface-400 transition-all flex items-center gap-2">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <button onClick={handleScan} disabled={!parsedText || isScoring}
                        className="px-6 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-primary-500 to-primary-600 rounded-xl hover:shadow-lg hover:shadow-primary-500/20 hover:-translate-y-0.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:transform-none flex items-center gap-2">
                        {isScoring ? (
            ...
            <AnimatePresence>
                {isScoring && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                        className="mt-8 p-8 bg-white rounded-2xl border border-surface-300 text-center">
                        <div className="w-16 h-16 mx-auto mb-4 border-3 border-primary-200 border-t-primary-500 rounded-full animate-spin" />
                        <p className="text-surface-950 font-semibold mb-1">Scanning Against 6 ATS Platforms...</p>
                        <p className="text-xs text-surface-500">Workday · Taleo · iCIMS · Greenhouse · Lever · SuccessFactors</p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {parsedText && !isParsing && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                        className="mt-6 p-5 bg-white rounded-2xl border border-surface-300">
                        <div className="flex items-center gap-2 mb-4 text-primary-500">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                                <polyline points="14,2 14,8 20,8" />
                            </svg>
                            <h3 className="text-surface-950 font-bold text-sm">Resume Overview</h3>
                        </div>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-4">
                            {(() => {
                                ...
                                return stats.map((s) => (
                                    <div key={s.label} className="flex flex-col items-center p-2.5 bg-white/[0.02] border border-surface-200 rounded-xl">
                                        <span className="text-xl font-extrabold text-surface-950 tabular-nums">{s.value}</span>
                                        <span className="text-[10px] text-surface-500 uppercase tracking-wide mt-0.5">{s.label}</span>
                                    </div>
                                ));
                            })()}
                        </div>
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="mb-4 pt-3 border-t border-surface-200">
                                    <h4 className="text-xs font-semibold text-surface-600 mb-2">Detected Sections</h4>
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="mb-4 pt-3 border-t border-surface-200">
                                    <h4 className="text-xs font-semibold text-surface-600 mb-2">Extracted Skills ({skills.length})</h4>
                                    <div className="flex flex-wrap gap-1.5">
                                        {skills.slice(0, 30).map((skill) => (
                                            <span key={skill} className="px-2.5 py-0.5 text-[11px] font-medium text-primary-500 bg-primary-500/8 border border-primary-500/15 rounded-full">
                                                {skill}
                                            </span>
                                        ))}
                                        {skills.length > 30 && (
                                            <span className="px-2.5 py-0.5 text-[11px] font-medium text-surface-500 bg-white/[0.03] border border-surface-300 rounded-full">
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="mb-4 pt-3 border-t border-surface-200">
                                    <h4 className="text-xs font-semibold text-surface-600 mb-2">Contact Info</h4>
                                    <div className="flex flex-col gap-1.5">
                                        {emailMatch && (
                                            <div className="flex items-center gap-2 text-xs text-surface-700">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-surface-500 flex-shrink-0">
                                        ...
                                        {linkedinMatch && (
                                            <div className="flex items-center gap-2 text-xs text-surface-700">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-surface-500 flex-shrink-0">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {[
                                    ...
                                    <div key={flag.label} className="flex items-center gap-1.5 text-xs">
                                        ...
                                        <span className={flag.ok ? "text-surface-600" : "text-amber-400"}>{flag.label}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <AnimatePresence>
                {hasResults && !isScoring && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 space-y-6">
                        {/* Summary Header */}
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 bg-white rounded-2xl border border-surface-300">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-surface-500 mt-0.5">Average ATS Score</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="text-right">
                                    <span className="text-xl font-bold text-surface-950">{passCount}/{results.length}</span>
                                    <p className="text-xs text-surface-500">Systems Passed</p>
                                </div>
                                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${jobDescription.trim() ? "bg-primary-50 border border-primary-200 text-primary-500" : "bg-surface-200 border border-surface-300 text-surface-600"
                                    }`}>
                                    {jobDescription.trim() ? "🎯 Targeted" : "🛡️ General"}
                                </span>
                                <button onClick={exportResults} className="p-2 bg-surface-200 border border-surface-300 rounded-lg text-surface-600 hover:text-surface-800 hover:border-surface-400 transition-all" title="Export JSON">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        ...
                        {allSuggestions.length > 0 && (
                            <div className="p-6 bg-white rounded-2xl border border-surface-300">
                                <div className="flex items-center gap-2 mb-1 text-primary-500">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z" />
                                    </svg>
                                    <h3 className="text-surface-950 font-bold">Optimization Suggestions</h3>
                                </div>
                                <p className="text-xs text-surface-500 mb-4">Actionable recommendations based on analysis across all 6 ATS platforms.</p>
                                    ...
                                    {allSuggestions.map((suggestion, i) => {
                                        ...
                                        return (
                                            <button key={i} onClick={() => setExpandedSuggestion(expandedSuggestion === i ? null : i)}
                                                className="w-full text-left p-4 bg-white/[0.02] border border-surface-200 rounded-xl hover:border-surface-300 transition-all">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="flex-1 text-sm text-surface-700">{structured ? suggestion.summary : String(suggestion)}</span>
                                                    <span className="text-[10px] font-semibold uppercase" style={{ color: impactColor }}>{impactLabel}</span>
                                                    <svg className={`w-3.5 h-3.5 text-surface-500 transition-transform ${expandedSuggestion === i ? "rotate-180" : ""}`}
                                                ...
                                                <AnimatePresence>
                                                    {expandedSuggestion === i && structured && (suggestion as StructuredSuggestion).details.length > 0 && (
                                                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                                                            <ul className="mt-3 pt-3 border-t border-surface-200 space-y-2">
                                                            ...
                                                            {(suggestion as StructuredSuggestion).platforms.length > 0 && (
                                                                <div className="flex gap-1.5 mt-2">
                                                                    {(suggestion as StructuredSuggestion).platforms.map((p) => (
                                                                        <span key={p} className="text-[10px] px-2 py-0.5 bg-surface-200 border border-surface-300 rounded text-surface-500">{p}</span>
                                                                    ))}
                                                                </div>
                                                            )}
                                                        </motion.div>
